...
/*
$sql=mysql_query("select ExpId from y1_expenseremark order by ExpId asc"); 
while($res=mysql_fetch_assoc($sql))
{
 $up=mysql_query("update y1_expenseclaims set Rmk=1 where ExpId=".$res['ExpId']);
}

$sql2=mysql_query("select ExpId from y2_expenseremark order by ExpId asc"); 
while($res2=mysql_fetch_assoc($sql2))
{
 $up2=mysql_query("update y2_expenseclaims set Rmk=1 where ExpId=".$res2['ExpId']);
}
*/
?>
	...
	<!-- <div id="claimerdiv" class="col-lg-10  col-md-10" style="border-left:5px solid #d9d9d9;"> -->
	 <div class="row">
      <div class="col-lg-12"><br />
	   ...
	   $q_all = "SELECT count(*) AS countT FROM `y".$_SESSION['FYearId']."_expenseclaims`e, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and (e.ClaimStatus='Submitted' or e.ClaimStatus='Filled' or e.ClaimStatus='Draft') and AttachTo=0"; $seleq_all=mysql_query($q_all);
	  ...
	  <?php $q_Ex2= "SELECT count(*) AS countT2 FROM y".$_SESSION['FYearId']."_expenseclaims e, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and  e.ClaimStatus!='Deactivate' and e.ClaimStatus='Submitted' and e.Rmk=1 and AttachTo=0"; $seleq_Ex2=mysql_query($q_Ex2);
      ...
      <?php $q_count = "SELECT e.ClaimStatus, count(*) AS Count FROM `y".$_SESSION['FYearId']."_expenseclaims`e, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and e.ClaimStatus!='Deactivate' and e.ClaimStatus='Draft' and Filledokay!=2 and AttachTo=0"; 
      ...
      <?php $q_count2 = "SELECT e.ClaimStatus, count(*) AS Count FROM `y".$_SESSION['FYearId']."_expenseclaims`e, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and e.ClaimStatus!='Deactivate' and e.ClaimStatus='Filled' and Filledokay!=2 and AttachTo=0 and e.BillDate>='".$mn."'"; 
      ...
      <?php $q_count3 = "SELECT e.ClaimStatus, count(*) AS Count FROM `y".$_SESSION['FYearId']."_expenseclaims`e, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and e.ClaimStatus!='Deactivate' and e.ClaimStatus='Submitted' and e.Rmk=0 and AttachTo=0"; $seleq_month3=mysql_query($q_count3); $rrs3 = mysql_fetch_assoc($seleq_month3); ?>       	
     ...
     <?php $q_Ex = "SELECT count(*) AS count FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and FilledOkay=2 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and ClaimStatus='Filled' and FilledBy>0 and AttachTo=0"; $seleq_Ex=mysql_query($q_Ex); $rrs_Ex = mysql_fetch_assoc($seleq_Ex); ?>
...
<script>
function Funve(ve,sts)
{
  window.location="home.php?sts="+sts+"&ve="+ve;
}
</script>	 
	     <?php if($_REQUEST['sts']!=''){$stst=$_REQUEST['sts'];}else{$stst='Submitted';} ?> 
		 <?php if($_REQUEST['ve']!=''){$ve=$_REQUEST['ve'];}else{$ve=0;} ?> 
	    <select style="font-size:14px; width:200px;" id="SelMonth" onchange="Funve(this.value,'<?=$stst;?>')">
		          
			      <option value="0" <?php if($_REQUEST['ve']=='' || $_REQUEST['ve']==0){echo 'selected';}?>>All</option>
...
$seleqe=mysql_query("SELECT e.CrBy,h.Fname,h.Sname,h.Lname, CASE WHEN e.ClaimId != 0  THEN BillDate WHEN e.ClaimId = 0  THEN CrDate END AS finalDate  FROM `y".$_SESSION['FYearId']."_expenseclaims`e, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and e.ClaimStatus!='Deactivate' ". $sts." GROUP BY e.CrBy ORDER BY h.Fname ASC"); while($expe=mysql_fetch_assoc($seleqe)){ 
?>
 <option value="<?=$expe['CrBy']?>" <?php if($_REQUEST['ve']==$expe['CrBy']){echo 'selected';}?>><?=$expe['Fname'].' '.$expe['Sname'].' '.$expe['Lname']?></option>
<?php } ?>								  
				  
				  
		 </select>
		 
		 &nbsp;&nbsp;<font style="font-size:18px; font-family:'Times New Roman', Times, serif; color:#FF0000;"><?=$_REQUEST['sts']?></font> 
...
if($ve>0){ $sqr=' AND e.CrBy='.$ve;}else{$sqr='';}
				...
				END AS finalDate  FROM `y".$_SESSION['FYearId']."_expenseclaims`e, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and e.ClaimStatus!='Deactivate' ".$sts." ".$sqr." GROUP BY ClaimMonth ORDER BY finalDate ASC";
	  	 	// print_r($qmonth);die();
			    ...
			    $totcount =0;
                 ...
                 $totcount =  $totcount + $exp_month['Count'];
			  		...
			  		<?php /*<td rowspan="<?=($exp_month['Count'] * 2)?>" style="font-weight: bold; text-transform: uppercase; color:#682d3d; text-align: center;"><?=$j."<br>".$month_name." ".date('Y', strtotime($exp_month['finalDate'])); ?></td>*/ ?>
          ...
          <?php
				...
				*/
				...
				END AS finalDate 
			  	 ...
			  	 FROM `y".$_SESSION['FYearId']."_expenseclaims`e, claimtype c, ".dbemp.".hrm_employee h where h.EmployeeID=e.CrBy and (c.ClaimId=e.ClaimId or e.ClaimId=0) and ".$stepcond." and e.ClaimMonth = '".$exp_month['ClaimMonth']."' ".$sts." ".$sqr." group by e.ExpId order by finalDate, EmployeeID";
