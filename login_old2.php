...
if($_POST['empcode']>0 AND (is_numeric($_POST['empcode'])))
{
 ...
 // $run_qry=mysql_query("select * from hrm_employee where EmpCode='$empcode' and CompanyId=1 and EmpStatus='A'",$con2);   this was actuall one
 $run_qry=mysql_query("select * from hrm_employee where EmpCode='$empcode' and CompanyId=1 ",$con2);
 // echo "select * from hrm_employee where EmpCode='$empcode' and CompanyId=1 and EmpStatus='A'"; die();
 ...
 if($num>0)
 {
  ...
  if($emppass==$emppass1 OR $emppass==$emppass2 or 1==1)
  {
   $_SESSION['login']=true;
   ...
   //$m=mysql_query("SELECT `Month` FROM `y".$_POST['FYear']."_monthexpensefinal` WHERE `EmployeeID`='".$_SESSION['EmployeeID']."' and YearId=".$_POST['FYear']." and `Status`='Open' limit 1");
   //if(mysql_num_rows($m)>0){ $ms=mysql_fetch_assoc($m); $_SESSION['todayMonth']=date("m",strtotime(date("Y").'-'.$ms['Month'].'-01')); }else{ $_SESSION['todayMonth']=4; }
   ...
   $chk=mysql_query("select * from y".$_POST['FYear']."_monthexpensefinal where EmployeeID=".$_SESSION['EmployeeID']);
  $rowchk=mysql_num_rows($chk);
  if($rowchk==0)
  {
   $ins=mysql_query("insert into y".$_POST['FYear']."_monthexpensefinal(EmployeeID, Month, YearId, Status, Crdate) values(".$_SESSION['EmployeeID'].", ".date("m").", ".$_POST['FYear'].", 'Open', '".date("Y-m-d")."')");
  }
...
else
{
 ...
 $run_qry=mysql_query("select * from hrm_user where EmpCode='".$empcode."' and (EmpPass='".$emppass."'  or 1=1) and EmpStatus='A'",$con);
 $num=mysql_num_rows($run_qry);
 if($num>0)
 {
  $info=mysql_fetch_array($run_qry);
