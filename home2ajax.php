...
if($_POST['act']=='okayAllFilledClaims')
{
	...
	while($cgd=mysql_fetch_assoc($cg)){ $claimids.=$cgd['ClaimId'].","; }
	$claimids.="123456";
...
elseif($_POST['act']=='submitmonthfill')
{
  ...
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET FilledOkay=1, ClaimAtStep=3, ApprTAmt=FilledTAmt where CrBy='".$_POST['crby']."' and ClaimMonth='".$closingMonth."' and ClaimYearId='".$_SESSION['FYearId']."' and ClaimStatus='Filled'");
  ...
  while($selexpd=mysql_fetch_assoc($selexp)){ mysql_query("update y".$_SESSION['FYearId']."_expenseclaimsdetails set ApproverEditAmount=Amount where ExpId='".$selexpd['ExpId']."'"); }
...
elseif($_POST['act']=='expfillok')
{
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET FilledOkay=1,FilOkDenyRemark='".$_POST['remark']."' where ExpId='".$_POST['expid']."'"); if($up){ echo 'okay'; }
}
elseif($_POST['act']=='expfilldeny')
{
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET FilledOkay=2,FilOkDenyRemark='".$_POST['remark']."' where ExpId='".$_POST['expid']."'"); if($up){ echo 'okay'; }	
}
elseif($_POST['act']=='deactivateclaim')
{
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET ClaimStatus='Deactivate' where ExpId='".$_POST['expid']."'"); if($up){ echo 'deactivated'; }
}
elseif($_POST['act']=='attach')
{
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET AttachTo='".$_POST['attachid']."' where ExpId='".$_POST['expid']."'"); if($up){ echo 'attached'; }
}
...
/******* Approver Start ******************//******* Approver Start ******************/
  ...
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET ClaimAtStep=4, ClaimStatus='Approved', ApprBy='".$_SESSION['EmployeeID']."', ApprDate='".date("Y-m-d")."' where CrBy='".$_POST['crby']."' and ClaimMonth='".$_POST['month']."' and ClaimYearId='".$_SESSION['FYearId']."' and (ClaimStatus='Filled' OR ClaimStatus='Approved')"); 
...
{
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_monthexpensefinal SET Status='Open', Approved_Amount=0 where EmployeeID='".$_POST['crby']."' and Month='".$_POST['month']."' and YearId='".$_SESSION['FYearId']."'"); 
  if($up){ echo 'Returned'; }
...
/******* Payment Start ******************//******* Payment Start ******************/
 ...
 $FinanceAmt=$_POST['AdvAmt']+$_POST['PayAmt'];
 //$up=mysql_query("update y".$_SESSION['FYearId']."_monthexpensefinal set Fin_AdvancePay='".$_POST['AdvAmt']."', Fin_PayOption='".$_POST['PayOption']."', Fin_PayBy=".$_SESSION['EmployeeID'].", Fin_PayAmt ='".$_POST['PayAmt']."', Fin_PayDate='".date("Y-m-d", strtotime($_POST['PayDate']))."', Fin_PayRemark='".$_POST['PayRemark']."', Finance_Amount='".$_POST['PayAmt']."', Finance_Date='".date("Y-m-d")."' where EmployeeID='".$_POST['crby']."' and Month='".$_POST['month']."' and YearId='".$_SESSION['FYearId']."'");
 ...
 $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET ClaimStatus='Payment', ClaimAtStep=5, VerifyTAmt=ApprTAmt where CrBy='".$_POST['crby']."' and ClaimMonth='".$_POST['month']."' and ClaimYearId='".$_SESSION['FYearId']."' and ClaimStatus='Approved'");
...
/******* Verifier Start ******************//******* Verifier Start ******************/
   ...
   $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET ClaimStatus='Financed', FinancedBy='".$_SESSION['EmployeeID']."', `FinancedTAmt`=`VerifyTAmt`, `FinancedDate`='".date("Y-m-d")."', ClaimAtStep=6 where CrBy='".$_POST['crby']."' and ClaimMonth='".$_POST['month']."' and ClaimYearId='".$_SESSION['FYearId']."' and ClaimStatus='Verified'");
   ...
   /******** ---- final table update ************/
   $Vpaid=mysql_query("SELECT SUM(VerifyTAmt) as VerifyAmt, MAX(VerifyDate) as VerifyD, SUM(FinancedTAmt) as FinAmt, MAX(FinancedDate) as FinD FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_POST['crby']."' and ClaimMonth='".$_POST['month']."' and FilledOkay=1 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and VerifyBy>0"); 
   ...
   if($clm2['FinAmt']>0){$FinAmt=$clm2['FinAmt'];}else{$FinAmt=0;} 
   if($clm2['FinD']!=''){$FinD=$clm2['FinD'];}else{$FinD='0000-00-00';}
  
   $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_monthexpensefinal SET Verified_Amount=".$Verified_Amount.", Verified_Date='".$Verified_Date."', Finance_Amount='".$FinAmt."', Finance_Date='".$FinD."', Fin_AppBy=".$_SESSION['EmployeeID'].", Fin_AppDate='".date("Y-m-d")."' where EmployeeID='".$_POST['crby']."' and Month='".$_POST['month']."' and YearId='".$_SESSION['FYearId']."'");
   /******** ---- final table update ************/
...
{
  $up=mysql_query("UPDATE y".$_SESSION['FYearId']."_expenseclaims SET ClaimStatus='Verified', VerifyBy='".$_SESSION['EmployeeID']."', `VerifyTAmt`=`ApprTAmt`, `VerifyDate`='".date("Y-m-d")."' where CrBy='".$_POST['crby']."' AND (VerifyBy=0 OR VerifyBy='') AND ClaimMonth='".$_POST['month']."' AND (ClaimStatus='Approved' OR ClaimStatus='Payment') AND (ClaimAtStep=4 OR ClaimAtStep=5)");
