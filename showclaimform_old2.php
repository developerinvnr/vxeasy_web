...
//$li2=mysql_query("SELECT Travel_TwoWeeKM, Travel_FourWeeKM,FourWElig,CostOfVehicle,WithDriver,AdvanceCom,DateOfEntryPolicy,LessKm,Plan FROM `hrm_employee_eligibility` where EligibilityId=(select MAX(EligibilityId) as MaxId from hrm_employee_eligibility where EligCreatedDate<'2020-01-01' AND EmployeeID=".$exp['CrBy'].") AND EmployeeID=".$exp['CrBy'],$con2);
//$lim2=mysql_fetch_assoc($li2); $tpkm2=$lim2['Travel_TwoWeeKM']; $fpkm2=$lim2['Travel_FourWeeKM'];
...
<input type="hidden" id="fw" value="<?=$fpkm?>" /><input type="hidden" id="fw2" value="<?=$fpkm2?>" />
						...
						?>
						<input type="text" class="form-control" id="tpkm" name="tpkm" value="<?=$tpkm?>" required>
						<input type="text" class="form-control" id="fpkm" name="fpkm" value="<?=$fpkm?>" required style="display: none;">
					</td>	
