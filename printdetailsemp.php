		 ...
		 <td style="width:100%; vertical-align:top;" cellspacing="0">
		  <table style="width:100%;" border="1" cellspacing="0">
		   <tr style="height:25px;background-color:#0062C4;color:#FFFFFF;">
			...
			<th scope="col" style="width:80px;font-size:12px;">Date</th>
			<th scope="col" style="width:200px;font-size:12px;">Remark</th>
			
			<?php if($rGp['cgId']==1){ ?>
			<th scope="col" style="width:60px;font-size:12px;">FromKm</th>
			<th scope="col" style="width:60px;font-size:12px;">ToKm</th>
			<th scope="col" style="width:60px;font-size:12px;">TotalKm</th>
			<?php } ?>
		   ...
		   <?php $sDetl=mysql_query("SELECT BillDate,Remark,SUM(FinancedTAmt) as Tot2Amt, SUM(FilledTAmt) as Tot2Fill FROM `y".$_REQUEST['y']."_expenseclaims` WHERE FilledBy>0 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." and ClaimId in (".$qry.") and ".$sqry." and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' group by BillDate order by BillDate asc"); 
		   ...
		   <tr style="height:25px;">
			...
			if($rDetl['Remark']==''){ $sDetl2l=mysql_query("SELECT Remark FROM `y".$_REQUEST['y']."_expenseclaims` WHERE FilledBy>0 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." and BillDate='".date("Y-m-d",strtotime($rDetl['BillDate']))."' and Remark!='' and ClaimStatus!='Deactivate' and ClaimStatus!='Draft'"); 
		   $rDetl2l=mysql_fetch_assoc($sDetl2l); }
			
			?>
			<td align="left" style="font-size:12px;"><?php if($rDetl['Remark']!=''){ echo substr_replace(ucfirst(strtolower($rDetl['Remark'])), '', 35).''; if($rDetl['Remark']!=''){echo '...';} }else{ echo substr_replace(ucfirst(strtolower($rDetl2l['Remark'])), '', 35).''; if($rDetl2l['Remark']!=''){echo '...';} } ?></td>
			<?php } ?>
			
			
			<?php if($rGp['cgId']==1){ ?>
			<?php $rowctyp=0; $rowwf=0; $sctyp=mysql_query("select ExpId,ClaimId from `y".$_REQUEST['y']."_expenseclaims` where BillDate='".$rDetl['BillDate']."' and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." and ClaimId=7 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft'"); $rowctyp=mysql_num_rows($sctyp); 
			if($rowctyp>0)
			{
			 $resctyp=mysql_fetch_assoc($sctyp);
			 if($resctyp['ClaimId']==7)
			 { 
			  $swf=mysql_query("SELECT DistTraOpen,DistTraClose,Totalkm FROM `y".$_REQUEST['y']."_24_wheeler_entry` WHERE ExpId=".$resctyp['ExpId']); $rowwf=mysql_num_rows($swf); 
			  if($rowwf==1)
			  { $reswf=mysql_fetch_assoc($swf); 
			    $wf=$reswf['DistTraOpen']; $wt=$reswf['DistTraClose']; $wtot=$reswf['Totalkm']; }
			  else{ $wf=0; $wt=0; $wtot=0; } ?>
			<td align="center" style="font-size:12px;"><?php if($wf>0){ echo $wf; }?></td>
			<td align="center" style="font-size:12px;"><?php if($wt>0){ echo $wt; }?></td>
			<td align="center" style="font-size:12px;"><?php if($wtot>0){ echo $wtot; }?></td>
			<?php  if($wtot>0){ $totKmAmt += $wtot; } 
			 } //if($resctyp['ClaimId']==7)
			}else{ ?><td colspan="3" style="font-size:12px;"></td><?php } ?>
			<?php } //if($rGp['cgId']==1){ ?>
