...
<td colspan="6" style="text-align:justify;">
    <?php
	$grpscount = array();
	$msts=mysql_query("select Status from `y".$_SESSION['FYearId']."_monthexpensefinal` where Month='".$_POST['month']."' and EmployeeID='".$_SESSION['EmployeeID']."'");
	...
	{ ?><button class="btn btn-outline-primary btn-sm pull-left grpbtns" style="font-weight:bold;height:25px;font-size:10px;margin-left:2px;" onclick="clShowDaysOfMonth(this,'<?=$_POST['month']?>','<?=$_SESSION['EmployeeID']?>','<?=$cged['cgId']?>','<?=$mstsd['Status']?>','Filled')" style="" ><?=$cged['cgCode']?> <span class="badge badge-danger" style="font-size:10px;"><?=$grpscount['g'.$cged['cgId']]?></span></button>
	<?php
    }
	?>
</td>
	...
	<table class="table shadow" style="width:100%;" align="center">
	  <thead class="thead-dark">
	   <tr>
		...
		<?php if(isset($_POST['msts']) && $_POST['msts']=='Closed'){ ?>
		...
		<?php if(isset($_POST['cgid']) && $_POST['cgid']!=0 && isset($_POST['msts']) && $_POST['msts']!='Closed'){ ?>
		 <label for="okayall<?=$_POST['cgid']?>">
		 <span id="okbtn<?=$exp['ExpId']?>" class="btn btn-sm btn-success" style=""><input id="okayall<?=$_POST['cgid']?>" type="checkbox" onclick="okayall('<?=$_REQUEST['emp']?>','<?=$_REQUEST['month']?>','<?=$_POST['cgid']?>')"><font style="font-size:10px;">Okay All</font></span>
		  </label>
		 <?php } ?>
		 </th>
	    </tr>
     ...
     <?php  if($_POST['cgid']==0 || $_POST['csts']=='Draft'){
	    ...
	    <tr onclick="showdet('<?=$exp['EmployeeID']?>')" >
	       <th scope="row">
			...
			<td>
		   	...
		   	<td class="text-center" style="vertical-align:middle;">
		   		...
		   		$totpaida=mysql_query("SELECT SUM(FilledTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` e, claimtype c WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_SESSION['EmployeeID']."' and c.ClaimId=e.ClaimId  and c.cgId='".$_POST['cgid']."' and e.CrDate='".$exp['finalDate']."' and e.ClaimStatus='".$_POST['csts']."' and e.FilledOkay!=2"); 
			   		...
			   		$totpaida=mysql_query("SELECT SUM(FilledTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` e, claimtype c WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_SESSION['EmployeeID']."' and c.ClaimId=e.ClaimId  and c.cgId='".$_POST['cgid']."' and e.BillDate='".$exp['finalDate']."' and e.ClaimStatus!='Deactivate' and e.FilledBy>0 and e.FilledOkay!=2"); 
			...
			<td class="text-right">
		   		<font style="font-size:11px;">
		   		<?php 
		   		$totpaid=mysql_query("SELECT SUM(FinancedTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` e, claimtype c WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_SESSION['EmployeeID']."' and c.ClaimId=e.ClaimId  and c.cgId='".$_POST['cgid']."' and e.BillDate='".$exp['finalDate']."' and e.ClaimStatus!='Deactivate' and e.FilledBy>0 and e.FinancedBy>0 and e.FinancedDate!='0000-00-00'"); 
			...
			<td class="text-right">
				...
				<?php 
		   		...
		   		$filla=mysql_query("SELECT CrDate FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_SESSION['EmployeeID']."' and BillDate='".$exp['finalDate']."' and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and FilledBy>0 and AttachTo=0"); 
		  		$fill=mysql_num_rows($filla);	

		  		$totpaid=mysql_query("SELECT CrDate FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_SESSION['EmployeeID']."' and BillDate='".$exp['finalDate']."' and FilledOkay=1 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and FilledBy>0 and AttachTo=0"); 
		  		$ok=mysql_num_rows($totpaid);	

		  		$totpaid=mysql_query("SELECT CrDate FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_SESSION['EmployeeID']."' and BillDate='".$exp['finalDate']."' and FilledOkay=2 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and FilledBy>0 and AttachTo=0"); 
	...
	<table class="table shadow" style="width:100%;" align="center">
	  <thead class="thead-dark">
	    <tr>
			...
			<th scope="col" style="background-color:#008C8C;width:50px;"><font style="font-size:11px;"></th>
	  ...
	  <tbody>
	    ...
	    <tr onclick="showdet('<?=$exp['EmployeeID']?>')" style="<?=$trcolor?>">
		  ...
		  <td class="text-right">
			 ...
			 <?php } ?>
		      <?php //if($exp['VerifyDate']!='0000-00-00' AND $exp['VerifyBy']>0){ echo $exp['VerifyTAmt']; }?>
		  </td>
		  <td class="text-left"><?php if($exp['VerifyDate']!='0000-00-00' AND $exp['VerifyBy']>0){ echo $exp['VerifyTRemark']; } ?></td>
...
<!--<td></td>-->
 ...
 <tbody>
<?php
if($_REQUEST['csts']=='Submitted'){ $stepcond="ClaimAtStep>=3"; }
elseif ($_REQUEST['csts']=='Approved'){ $stepcond="ClaimAtStep>=4"; }
?> 
 <tr class="totalrow">
  <th scope="row" colspan="6" class="text-right"><b>Total&nbsp;</b></th>
   ...
   <?php /*?><td class="text-right"><font style="font-size:11px;"><?php $totpaid2=mysql_query("SELECT SUM(VerifyTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` e WHERE `ClaimMonth`='".$_REQUEST['month']."' and `ClaimYearId`='".$_SESSION['FYearId']."' and e.CrBy=".$_REQUEST['emp']." and FilledOkay=1 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and VerifyBy>0"); $clm2=mysql_fetch_assoc($totpaid2); echo $clm2['paid']; ?></font></td>
   <td class="text-right">&nbsp;</td><?php */?>
...
<?php 
if($_REQUEST['csts']=='Verified'){ $stepcond="ClaimAtStep>=3"; }
elseif ($_REQUEST['csts']=='Approved'){ $stepcond="ClaimAtStep>=4"; }
$crcond="1=1";
...
<?php //$famt=intval($exp['FilledTAmt']); $vamt=intval($exp['VerifyTAmt']);
  ...
  <?php /*?><td <?=$vcstyle?> class="text-right"><?=$exp['VerifyTAmt']?></td>
  <td class="text-left"><?=$exp['VerifyTRemark']?></td><?php */?>
  ...
  <td><?php if($exp['ClaimStatus']=='Submitted'){$c='outline-warning'; $s='P'; }elseif($exp['ClaimAtStep']>=4){$c='outline-success'; $s='A'; } ?>
...
// if($s=='Approved' && ($exp['ClaimAtStep']==4 || $exp['ClaimAtStep']==5)){$c='success'; $btn='View';} ?>
  ...
  <td>
<?php if($exp['ClaimStatus']!='Approved' && $exp['ClaimAtStep']<=3){ ?>
  <span id="appraction">
	  ...
	  <thead class="thead-dark">
	    <tr>
	      ...
	      <th scope="row" class="text-center table-active"  style=""><span style="font-size: 10px !important;">Claimed</span><br>Amt</th>
			...
			<th scope="row" class="text-center table-active" ><span style="font-size: 10px !important;">Verifier</span><br>Remark</th>-->
...
<?php
if($_REQUEST['csts']=='Closed'){$stepcond="e.ClaimAtStep>=5";
}else{ $stepcond="e.ClaimAtStep>=4"; }
?> 
 <tr class="totalrow">
  <th scope="row" colspan="5" class="text-right"><b>Total&nbsp;</b></th>
   ...
   <?php /*?><td class="text-right"><font style="font-size:11px;"><?php $totpaid4=mysql_query("SELECT SUM(FinancedTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` e WHERE `ClaimMonth`='".$_REQUEST['month']."' and `ClaimYearId`='".$_SESSION['FYearId']."' and e.CrBy=".$_REQUEST['emp']." and FilledOkay=1 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and FinancedBy>0"); $clm4=mysql_fetch_assoc($totpaid4); //echo $clm4['paid']; ?></font>
   ...
   <th colspan="4"></th><?php */?>
	    ...
	    <tr onclick="showdet('<?=$exp['EmployeeID']?>')">
	      ...
	      <td><?=$exp['ExpId']?></td>
	      <!-- <td><?=$exp['ExpenseName']?></td> -->
	      <td style="width:150px;"><input type="text" style="border:hidden;width:99%;" value="<?=$exp['ClaimName']?>" /></td>
	      ...
	      <td>
			...
			<td>
				<textarea style="width:<?php if($exp['VerifyTRemark']!=''){echo 80;}else{echo 50;} ?>px;height:<?php if($exp['VerifyTRemark']!=''){echo 75;}else{echo 25;} ?>px;"  class="form-control  text-left" id="<?=$exp['ExpId']?>verifiedtremark" rows="1" readonly><?=$exp['VerifyTRemark']?></textarea>
			</td> <?php */?>
			...
			<td>
				<textarea readonly style="width: <?php if($exp['ApprTRemark']!=''){echo 80;}else{echo 50;} ?>px;height: <?php if($exp['ApprTRemark']!=''){echo 75;}else{echo 25;} ?>px;"  class="form-control  text-left" id="<?=$exp['ExpId']?>apprtremark" rows="1"><?=$exp['ApprTRemark']?></textarea>
			</td> 
			...
			<td>
				<textarea style="width: 100px;height: <?php if($exp['FinancedTRemark']!=''){echo 75;}else{echo 25;} ?>px;"  class="form-control  text-left" id="<?=$exp['ExpId']?>financetremark" rows="1"><?=$exp['FinancedTRemark']?></textarea>
			</td><?php */?>
	      <td>
	      	<?php
			...
			//else { $s=''; }
			if($exp['ClaimStatus']=='Payment'){$s='Paid';}
			elseif($exp['ClaimStatus']=='Approved'){$s='Approved';}
			elseif($exp['ClaimStatus']=='Financed'){$s='Financed';}
			else { $s='Pending'; }
	      	?>
	      ...
	      <?php
	      if($_REQUEST['sts']=='Open' AND $exp['ClaimStatus']!='Approved' AND $exp['ClaimStatus']!='Payment' AND $exp['ClaimStatus']!='Rej_Fin' AND $_SESSION['EmpRole']!='P'){
	      ?>
	...
	<table class="table shadow">
	  <thead class="thead-dark">
	    <tr>
	      ...
	      <th scope="row" class="text-center table-active"  style=""><span style="font-size: 10px !important;">Approved</span><br>Amt</th>
	  ...
	  <tbody>
	  	<tr class="totalrow">
			...
			<td class="text-right">
				<?php
				$totpaid=mysql_query("SELECT SUM(ApprTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_REQUEST['emp']."' and ClaimMonth='".$_REQUEST['month']."' and FilledOkay=1 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and ApprBy>0");
	    ...
	    <tr onclick="showdet('<?=$exp['EmployeeID']?>')">
	      ...
	      <td><?=date("d-m-Y",strtotime($exp['BillDate']))?></td>
			...
			<td>
				<input style="width: 70px;" class="form-control text-right" type="text" id="claimedtamt" value="<?=$exp['ApprTAmt']?>" readonly>
			</td>
			<td>				
				<input style="width: 70px;"  class="form-control text-right" type="text" id="<?=$exp['ExpId']?>verifiedtamt" value="<?=$exp['VerifyTAmt']?>" onkeypress="return isNumber(event)" onkeyup="checkrange(this,'<?=$exp['ApprTAmt']?>',<?=$exp['ExpId']?>)">
			</td>
			<td>
	      ...
	      <td>
	      	...
	      	<input type="checkbox" onclick="showbtn(this,'<?=$exp['ExpId']?>')">
	      	<button id="<?=$exp['ExpId']?>verifybtn" class="btn btn-sm btn-success pull-right" onclick="verifyClaim('<?=$exp['ExpId']?>',<?=$_REQUEST['id']?>)" disabled>
		...
		<tr class="totalrow">
			...
			<td class="text-right">
				<?php
				$totpaid=mysql_query("SELECT SUM(ApprTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$_REQUEST['emp']."' and ClaimMonth='".$_REQUEST['month']."' and FilledOkay=1 and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and FilledBy>0");
	  			//$totpaid=mysql_query("SELECT SUM(FilledTAmt) as paid FROM `y".$_SESSION['FYearId']."_expenseclaims` WHERE `ClaimMonth`='".$_REQUEST['month']."' and `ClaimYearId`='".$_SESSION['FYearId']."' ");
	  			$clm=mysql_fetch_assoc($totpaid);
	  			echo $clm['paid'];
				?>
