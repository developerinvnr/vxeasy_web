...
<tr>
 <td style="width:95%;text-align:center; font-size:20px;">
...
$sTot=mysql_query("SELECT count(*) as TotClaim, SUM(FinancedTAmt) as TotFin FROM `y".$_REQUEST['y']."_expenseclaims` WHERE  `ClaimAtStep`=6 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m'].""); 
...
?>
   <tr style="font-size:14px; height:25px; background-color:#FFFFB3;">
    ...
    <td style="text-align:center;"><?=date("d-m-Y",strtotime($mlist['Fin_PayDate']));?></td>
	...
	<table border="1" cellspacing="0" style="width:100%;">
	  ...
	  <tbody>
        <tr class="totalrow" style="background-color:#FFFFD2;">
         <th scope="row" colspan="5" style="font-size:14px;text-align:right;">Total:&nbsp;</th>
         <td style="text-align:right;"><font style="font-size:12px; font-weight:bold;"><?php $totpaid=mysql_query("SELECT SUM(FilledTAmt) as paid FROM `y".$_REQUEST['y']."_expenseclaims` e WHERE `ClaimMonth`='".$_REQUEST['m']."' and `ClaimYearId`='".$_REQUEST['y']."'and e.CrBy=".$_REQUEST['e']." and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and FilledBy>0"); $clm=mysql_fetch_assoc($totpaid); echo $clm['paid']; ?></font>&nbsp;</td>
         <td style="text-align:right;"><font style="font-size:12px;font-weight:bold;"><?php $totpaid2=mysql_query("SELECT SUM(VerifyTAmt) as paid FROM `y".$_REQUEST['y']."_expenseclaims` e WHERE `ClaimMonth`='".$_REQUEST['m']."' and `ClaimYearId`='".$_REQUEST['y']."' and e.CrBy=".$_REQUEST['e']." and ClaimStatus!='Deactivate' and ClaimStatus!='Draft' and VerifyBy>0"); $clm2=mysql_fetch_assoc($totpaid2); echo $clm2['paid']; ?></font>&nbsp;</td>
         <td style="text-align:right;"><font style="font-size:12px;font-weight:bold;"><?php $totpaid3=mysql_query("SELECT SUM(ApprTAmt) as paid FROM `y".$_REQUEST['y']."_expenseclaims` e WHERE `ClaimMonth`='".$_REQUEST['m']."' and `ClaimYearId`='".$_REQUEST['y']."' and e.CrBy=".$_REQUEST['e']." and ClaimStatus!='Deactivate' and ApprBy>0"); $clm3=mysql_fetch_assoc($totpaid3); echo $clm3['paid']; ?></font>&nbsp;</td>
         <td style="text-align:right;"><font style="font-size:12px;font-weight:bold;"><?php $totpaid4=mysql_query("SELECT SUM(FinancedTAmt) as paid FROM `y".$_REQUEST['y']."_expenseclaims` e WHERE `ClaimMonth`='".$_REQUEST['m']."' and `ClaimYearId`='".$_REQUEST['y']."' and e.CrBy=".$_REQUEST['e']." and ClaimStatus!='Deactivate' and FinancedBy>0"); $clm4=mysql_fetch_assoc($totpaid4); echo $clm4['paid']; ?></font>&nbsp;</td>
        </tr>

	  	<?php $seleq=mysql_query("SELECT e.*, c.ClaimName, h.Fname,h.Sname,h.Lname FROM `y".$_REQUEST['y']."_expenseclaims`e, claimtype c, ".dbemp.".hrm_employee h where c.ClaimId=e.ClaimId and h.EmployeeID=e.CrBy and e.ClaimMonth='".$_REQUEST['m']."' and e.ClaimYearId='".$_REQUEST['y']."' and e.CrBy=".$_REQUEST['e']." and e.ClaimAtStep>=6 order by BillDate ASC");
...
<?php /****************************************************/ ?>
	...
	$sChk=mysql_query("SELECT * FROM `y".$_REQUEST['y']."_expenseclaims` WHERE ClaimId in (".$qry.") and `ClaimAtStep`=6 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']."");
		...
		<tr>
		 <td style="width:100%; vertical-align:top;" cellspacing="0">
		  <table style="width:100%;" border="1" cellspacing="0">
		   ...
		   <?php $sDetl=mysql_query("SELECT BillDate FROM `y".$_REQUEST['y']."_expenseclaims` WHERE `ClaimAtStep`=6 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." and ClaimId in (".$qry.") group by BillDate order by BillDate asc"); 
		   ...
		   <tr style="height:25px;">
			...
			$stot=mysql_query("select SUM(FinancedTAmt) as TotAmt from `y".$_REQUEST['y']."_expenseclaims` where ClaimId=".$rCtyp['ClaimId']." and BillDate='".$rDetl['BillDate']."' and `ClaimAtStep`=6 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." "); $rtot=mysql_fetch_assoc($stot);?>
			
			
			<?php if($rCtyp['ClaimId']==7){ $tot1=0; $tot2=0;
			if($rtot['TotAmt']>0)
			{
			$s2tot=mysql_query("select SUM(FilledTAmt) as TotAmt from `y".$_REQUEST['y']."_expenseclaims` clm inner join `y".$_REQUEST['y']."_g1_expensefilldata` gd on clm.ExpId=gd.ExpId where clm.ClaimId=".$rCtyp['ClaimId']." and clm.BillDate='".$rDetl['BillDate']."' and clm.ClaimStatus!='Deactivate' and clm.ClaimStatus!='Draft' and clm.FilledBy>0 and clm.CrBy=".$_REQUEST['e']." and clm.ClaimYearId=".$_REQUEST['y']." and clm.ClaimMonth=".$_REQUEST['m']." and gd.VehicleType=2 "); 
			$r2tot=mysql_fetch_assoc($s2tot); $tot1=$r2tot['TotAmt'];
			$s4tot=mysql_query("select SUM(FilledTAmt) as TotAmt from `y".$_REQUEST['y']."_expenseclaims` clm inner join `y".$_REQUEST['y']."_g1_expensefilldata` gd on clm.ExpId=gd.ExpId where clm.ClaimId=".$rCtyp['ClaimId']." and clm.BillDate='".$rDetl['BillDate']."' and clm.ClaimStatus!='Deactivate' and clm.ClaimStatus!='Draft' and clm.FilledBy>0 and clm.CrBy=".$_REQUEST['e']." and clm.ClaimYearId=".$_REQUEST['y']." and clm.ClaimMonth=".$_REQUEST['m']." and gd.VehicleType=4 "); 
			...
			<td align="right" style="font-size:14px;background-color:#FFFFB3;"><b><?php $stot2=mysql_query("select SUM(FinancedTAmt) as Tot2Amt from `y".$_REQUEST['y']."_expenseclaims` where BillDate='".$rDetl['BillDate']."' and `ClaimAtStep`=6 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." and ClaimId in (".$qry.")"); $rtot2=mysql_fetch_assoc($stot2); if($rtot2['Tot2Amt']>0){echo $rtot2['Tot2Amt'];}else{echo '';} ?>&nbsp;</b></td>
		   </tr>
		   ...
		   <tr style="background-color:#FFFFB3;height:25px;">
			...
			$stot3=mysql_query("select SUM(FinancedTAmt) as Tot3Amt from `y".$_REQUEST['y']."_expenseclaims` where ClaimId=".$rCtyp2['ClaimId']." and `ClaimAtStep`=6 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." and ClaimId in (".$qry.")"); $rtot3=mysql_fetch_assoc($stot3);?>
			...
			$s2tot3=mysql_query("select SUM(FilledTAmt) as Tot3Amt from `y".$_REQUEST['y']."_expenseclaims` clm inner join `y".$_REQUEST['y']."_g1_expensefilldata` gd on clm.ExpId=gd.ExpId where clm.ClaimId=".$rCtyp2['ClaimId']." and clm.ClaimStatus!='Deactivate' and clm.ClaimStatus!='Draft' and clm.FilledBy>0 and clm.CrBy=".$_REQUEST['e']." and clm.ClaimYearId=".$_REQUEST['y']." and clm.ClaimMonth=".$_REQUEST['m']." and gd.VehicleType=2"); 
			$r2tot3=mysql_fetch_assoc($s2tot3);
			$s4tot3=mysql_query("select SUM(FilledTAmt) as Tot3Amt from `y".$_REQUEST['y']."_expenseclaims` clm inner join `y".$_REQUEST['y']."_g1_expensefilldata` gd on clm.ExpId=gd.ExpId where clm.ClaimId=".$rCtyp2['ClaimId']." and clm.ClaimStatus!='Deactivate' and clm.ClaimStatus!='Draft' and clm.FilledBy>0 and clm.CrBy=".$_REQUEST['e']." and clm.ClaimYearId=".$_REQUEST['y']." and clm.ClaimMonth=".$_REQUEST['m']." and gd.VehicleType=4"); 
			...
			<td align="right" style="font-size:14px;"><b><?php $stot4=mysql_query("select SUM(FinancedTAmt) as Tot4Amt from `y".$_REQUEST['y']."_expenseclaims` where `ClaimAtStep`=6 and CrBy=".$_REQUEST['e']." and ClaimYearId=".$_REQUEST['y']." and ClaimMonth=".$_REQUEST['m']." and ClaimId in (".$qry.")"); $rtot4=mysql_fetch_assoc($stot4); if($rtot4['Tot4Amt']>0){echo $rtot4['Tot4Amt'];}else{echo '';} ?>&nbsp;</b></td>
		   </tr>
		  </table>
		 </td>
		</tr>
