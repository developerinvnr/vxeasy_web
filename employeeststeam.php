...
<style>.bhd{text-align:center;color:#FFF;font-size:14px;}</style>						
		...
		<table class="table shadow table-responsive" style="width:100%;max-height:400px;overflow:scroll;" id="ReportTable">
						 ...
						 <?php $stupTFa=mysql_query("select ClaimName,BillDate,FilledTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where c.FilledBy>0 AND c.FilledDate!='0000-00-00' AND c.FilledDate!='1970-01-01' AND c.CrBy=".$_REQUEST['ei']." AND c.ClaimMonth=".$resym['Month']." AND c.FilledOkay=0 order by c.BillDate ASC"); $no=1; while($rtupTFa=mysql_fetch_assoc($stupTFa)){ ?>
		...
		<table class="table shadow table-responsive" style="width:100%;max-height:400px;overflow:scroll;" id="ReportTable">
						 ...
						 <?php $stupTV=mysql_query("select ClaimName,BillDate,FilledTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where FilledBy>0 AND FilledDate!='0000-00-00' AND FilledDate!='1970-01-01' AND VerifyBy=0 AND (VerifyDate='0000-00-00' OR VerifyDate='1970-01-01') AND CrBy=".$_REQUEST['ei']." AND ClaimMonth=".$resym['Month']." AND FilledOkay=1 order by c.BillDate ASC"); $no2=1; while($rtupTV=mysql_fetch_assoc($stupTV)){ ?>
					    <tr style="height:22px;">
						  <td><?php echo $no2; ?></td>
