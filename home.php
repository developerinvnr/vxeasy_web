...
//echo 'aa='.$_SESSION['CompanyId'].' - '.$_SESSION['EmployeeID'];
...
if(($_SESSION['EmpRole']=='E' OR $_SESSION['EmpRole']=='A') AND $_SESSION['FYearId']>=2)
{
  $mnt=intval(date("m"));
  ...
  $mnt2=intval(date("m"))-1;
  if($mnt2>=5 AND $_SESSION['FYearId']==2)
  {
   $chk=mysql_query("select * from y".$_SESSION['FYearId']."_monthexpensefinal where Month=".$mnt2." AND EmployeeID=".$_SESSION['EmployeeID']); $rowchk=mysql_num_rows($chk);
   if($rowchk==0)
   {
     if(!$_SESSION['aact'])
     {   
     $ins=mysql_query("insert into y".$_SESSION['FYearId']."_monthexpensefinal(EmployeeID, Month, YearId, Status, Crdate) values(".$_SESSION['EmployeeID'].", ".$mnt2.", ".$_SESSION['FYearId'].", 'Open', '".date("Y-m-d")."')");
     }
   }
  }
  ...
  $mnt3=intval(date("m"))-2;
  if($mnt3>=7 AND $_SESSION['FYearId']==2)
  {
   $chk=mysql_query("select * from y".$_SESSION['FYearId']."_monthexpensefinal where Month=".$mnt3." AND EmployeeID=".$_SESSION['EmployeeID']); $rowchk=mysql_num_rows($chk);
   if($rowchk==0)
   {
    
     if(!$_SESSION['aact'])
     {   
     $ins=mysql_query("insert into y".$_SESSION['FYearId']."_monthexpensefinal(EmployeeID, Month, YearId, Status, Crdate) values(".$_SESSION['EmployeeID'].", ".$mnt3.", ".$_SESSION['FYearId'].", 'Open', '".date("Y-m-d")."')");
     }
   }
  }
...
if($_SESSION['EmpRole']=='E' OR $_SESSION['EmpRole']=='A')
{
  ...
  if($roweMM>0)
  {
    ...
    while($reM=mysql_fetch_assoc($eMM))
    {
	 ...
	 if($rAmt['TotalAmt']>0)
	 {
	  $qry=mysql_query("update y".$_SESSION['FYearId']."_expenseclaims set FilledTAmt='".$rAmt['TotalAmt']."' where ClaimId=7 AND CrBy=".$_SESSION['EmployeeID']." AND ExpId=".$reM['ExpId']." AND ClaimStatus='Filled' and VerifyBy=0");
     }
	}
...
<?php if($_SESSION['EmpRole']=='A'){ ?>
 <div class="row">
  <div class="col-lg-3" style="height:8px;"></div>
   ...
   &emsp;<?php if(!$_SESSION['aact']){ ?><input checked type="radio" style="cursor: pointer;"  id="Claimerradio" name="role" onclick="showdiv('claimerdiv','apprdiv')"><label style="cursor: pointer;" for="Claimerradio" onclick="showdiv('claimerdiv','apprdiv')">&nbsp;<font style="font-size:15px;">Claimer</font></label><?php } ?>
...
<?php if((isset($_REQUEST['show']) && $_REQUEST['show']=='appr') || $_SESSION['aact']==1){
  ?>
  <script type="text/javascript">
