...
<div class="container">
 	<div class="row">
		...
		<div class="col-md-12 h-100 " style="padding:0px;">
			...
			<table class="table shadow" style="width:100%;" align="center">
			  <thead class="thead-dark">
			    <tr>
					...
					<th scope="col" style="width:70px;background-color:#008C8C;"><font style="font-size:11px;">Claim ID</font></th>
					...
					<th scope="col" style="width:400px;background-color:#008C8C;"><font style="font-size:11px;">Claim<br>Uploads</font></th>
			  ...
			  <tbody>
				...
				$ExpMDate=date('Y-m-d', strtotime("-1 months", strtotime(date("Y-m-d"))));
				
				$cl=mysql_query("SELECT * FROM `y".$_SESSION['FYearId']."_expenseclaims`  WHERE `ClaimYearId`='".$_SESSION['FYearId']."' and `CrBy`='".$crby."' and ExpId!='".$_REQUEST['expid']."' and (ClaimStatus='Submitted' OR ClaimStatus='Draft') AND ClaimStatus!='Deactivate' AND CrDate>='".$ExpMDate."' order by CrDate, ExpId"); //AttachTo=0
			    ...
			    <tr onclick="showdet('<?=$clm['EmployeeID']?>')" style="<?=$trcolor?>">
					...
					<td style="text-align: left;">
						<input type="checkbox" onclick="attachthis(this,'<?=$_REQUEST['expid']?>','<?=$clm['ExpId']?>')" <?php if($_REQUEST['expid']==$clm['AttachTo']){echo 'checked';} ?>>

						<span id="res<?=$clm['ExpId']?>">
					  	
					  	<?php if($_REQUEST['expid']==$clm['AttachTo']){ ?>
					  	 <div class='btn btn-sm btn-success'>Attached </div>
						<?php } ?>
