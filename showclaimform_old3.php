...
$S1=mysql_query("select SUM(Totalkm) as tot1 from 2_4_wheeler_entry w ".$subQ." AND JourneyStartDt between '".date($y1."-04-01")."' AND '".date($y1."-06-31")."' "); $R1=mysql_fetch_assoc($S1);
$S2=mysql_query("select SUM(Totalkm) as tot2 from 2_4_wheeler_entry w ".$subQ." AND JourneyStartDt between '".date($y1."-07-01")."' AND '".date($y1."-09-30")."' "); $R2=mysql_fetch_assoc($S2);
$S3=mysql_query("select SUM(Totalkm) as tot3 from 2_4_wheeler_entry w ".$subQ." AND JourneyStartDt between '".date($y1."-10-01")."' AND '".date($y1."-12-31")."' "); $R3=mysql_fetch_assoc($S3);
$S4=mysql_query("select SUM(Totalkm) as tot4 from 2_4_wheeler_entry w ".$subQ." AND JourneyStartDt between '".date($y2."-01-01")."' AND '".date($y2."-03-31")."' "); $R4=mysql_fetch_assoc($S4);
?>
...
<tr>
	<td colspan="6" style="width:100%; padding:0px;">
		...
		<table class="table-bordered table-sm claimtable w-100 paddedtbl " style="width:100%;padding:0px;" cellspacing="0" cellpadding="0">
				...
				<tr>
				  <th>Bill_Date</th>
					<td><input type="text" class="form-control dat" id="BillDate1" name="BillDate" value="<?=!empty($expf['BillDate'])?$expf['BillDate']:''?>"  required style="width:80px;"></td>
				
					<th scope="row">Vehicle&nbsp;<span class="text-danger">*</span></th>
					<td >
						<label >
							<input type="radio" class="" id="vehicleType2" name="vehicleType" value="2" readonly required  onclick="vehTypeSel(2,<?=$rowe?>,<?=$exp['CrBy']?>,<?=$y1.','.$y2?>)" checked=""><b>2W</b>
						</label>&emsp;
					...
					<td>
						<label >
							<input type="radio" class="" id="vehicleType4" name="vehicleType" value="4" readonly required onclick="vehTypeSel(4,<?=$rowe?>,<?=$exp['CrBy']?>,<?=$y1.','.$y2?>)"><b>4W</b>
						</label>
					</td>
...
$sAmt=mysql_query("select SUM(FilledTAmt) as TotalAmt from 2_4_wheeler_entry where Totalkm>0 AND ExpId=".$_REQUEST['expid']);
