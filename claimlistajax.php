	...
	<table class="table shadow" style="width:100%;" align="center">
	  ...
	  <tbody>
	    ...
	    <tr onclick="showdet('<?=$exp['EmployeeID']?>')" style="<?=$trcolor?>">
	      ...
	      <td class="text-right"><?php if($exp['VerifyDate']!='0000-00-00' AND $exp['VerifyBy']>0){ echo $exp['VerifyTAmt']; }?></td>
	  ...
	  <tbody>
	    ...
	    <tr onclick="showdet('<?=$exp['EmployeeID']?>')">
	      ...
	      <td><?=date("d-m-Y",strtotime($exp['BillDate']))?></td>
			...
			<td>				
				<input style="width: 70px;"  class="form-control text-right" type="text" id="<?=$exp['ExpId']?>verifiedtamt" value="<?=$exp['VerifyTAmt']?>" onkeypress="return isNumber(event)" onkeyup="checkrange(this,'<?=$exp['FilledTAmt']?>',<?=$exp['ExpId']?>)">
			</td>
			<td>
...
<?php $famt=intval($exp['FilledTAmt']); $vamt=intval($exp['VerifyTAmt']);
  ...
  <td><?php if($exp['ClaimStatus']=='Verified'){$c='outline-warning'; $s='P'; }elseif($exp['ClaimAtStep']>=5){$c='outline-success'; $s='A'; } ?>
...
// if($s=='Approved' && ($exp['ClaimAtStep']==4 || $exp['ClaimAtStep']==5)){$c='success'; $btn='View';} ?>
  ...
  <td>
<?php if($exp['ClaimStatus']!='Approved' && $exp['ClaimAtStep']<=4){ ?>
  <span id="appraction">
  <input type="checkbox" onclick="showbtn(this,'<?=$exp['ExpId']?>')">
