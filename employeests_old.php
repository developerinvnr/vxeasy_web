...
function FunEmp(ei)
{
 window.location="employeests.php?ei="+ei;
}
</script>
	...
	<div class="row  d-flex justify-content-around">
		<div class="col-md-10">
			<div class="row filrow font-weight-bold">
					...
					<select id="empid" class="form-control" onChange="FunEmp(this.value)" style="width:100%;">
...
<?php $stupDrf=mysql_query("select count(*) as TotupDrf from `y".$_SESSION['FYearId']."_expenseclaims` where CrBy=".$_REQUEST['ei']." AND ClaimMonth=0 AND ClaimStatus!='Deactivate' "); $rtupDrf=mysql_fetch_assoc($stupDrf); ?>						
            ...
            <td style="text-align:center;vertical-align:middle;"><?php echo date("M",strtotime(date("Y-".$resym['Month']."-d"))); ?></td>       
...
<style>.bhd{text-align:center;color:#FFF;font-size:14px;}</style>						
		...
		<table class="table shadow table-responsive" style="width:100%;max-height:400px;overflow:scroll;" id="ReportTable">
						 ...
						 <?php $stupTF=mysql_query("select CrBy,Count(*) as TotupTF from `y".$_SESSION['FYearId']."_expenseclaims` where FilledBy=0 AND (FilledDate='0000-00-00' OR FilledDate='1970-01-01') AND CrBy=".$_REQUEST['ei']." AND ClaimMonth=".$resym['Month']."group by CrBy ASC"); $no=1; while($rtupTF=mysql_fetch_assoc($stupTF)){ ?>
		...
		<table class="table shadow table-responsive" style="width:100%;max-height:400px;overflow:scroll;" id="ReportTable">
						 ...
						 <?php $stupTFa=mysql_query("select ExpId,ClaimName,BillDate,FilledTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where c.FilledBy>0 AND c.FilledDate!='0000-00-00' AND c.FilledDate!='1970-01-01' AND c.CrBy=".$_REQUEST['ei']." AND c.ClaimMonth=".$resym['Month']." AND c.FilledOkay=0 order by c.BillDate ASC"); $no=1; while($rtupTFa=mysql_fetch_assoc($stupTFa)){ ?>
					    <tr style="height:22px;background-color:<?php //if($no%2==0){echo '#FFF';} ?>;">
                          ...
                          <?php if($_SESSION['EmpRole']=='S'){?> 
						  <td style="text-align:center;cursor:pointer;"><img src="images/delete.png" onclick="DeleExpC(<?=$rtupTFa['ExpId'].','.$_SESSION['FYearId']?>)"/><?=$rtupTFa['ExpId']?></td>
						  <?php } ?>
						</tr>
                        <?php $no++; }?>
						<?php $stupTFaT=mysql_query("select sum(FilledTAmt) as sFilledTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where c.FilledBy>0 AND c.FilledDate!='0000-00-00' AND c.FilledDate!='1970-01-01' AND c.CrBy=".$_REQUEST['ei']." AND c.ClaimMonth=".$resym['Month']." AND c.FilledOkay=0 order by c.BillDate ASC"); $rtupTFaT=mysql_fetch_assoc($stupTFaT); ?>	
						<tr style="height:22px;">
		...
		<table class="table shadow table-responsive" style="width:100%;max-height:400px;overflow:scroll;" id="ReportTable">
						 ...
						 <?php $stupTV=mysql_query("select ExpId,ClaimName,BillDate,FilledTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where FilledBy>0 AND FilledDate!='0000-00-00' AND FilledDate!='1970-01-01' AND VerifyBy=0 AND (VerifyDate='0000-00-00' OR VerifyDate='1970-01-01') AND CrBy=".$_REQUEST['ei']." AND ClaimMonth=".$resym['Month']." AND FilledOkay=1 order by c.BillDate ASC"); $no2=1; while($rtupTV=mysql_fetch_assoc($stupTV)){ ?>
					    <tr style="height:22px;">
						  <td><?php echo $no2; ?></td>
                        ...
                        <?php $no2++; } ?>	
						<?php $stupTVT=mysql_query("select sum(FilledTAmt) as sFilledTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where FilledBy>0 AND FilledDate!='0000-00-00' AND FilledDate!='1970-01-01' AND VerifyBy=0 AND (VerifyDate='0000-00-00' OR VerifyDate='1970-01-01') AND CrBy=".$_REQUEST['ei']." AND ClaimMonth=".$resym['Month']." AND FilledOkay=1 order by c.BillDate ASC"); $rtupTVT=mysql_fetch_assoc($stupTVT); ?>
						 <tr style="height:22px;">
					      <td colspan="3" style="text-align:right;"><b>Total</b>&nbsp;</td>	
		...
		<table class="table shadow table-responsive" style="width:100%;max-height:400px;overflow:scroll;" id="ReportTable">
						 ...
						 <?php $stupTA=mysql_query("select ClaimName,BillDate,FilledTAmt,VerifyTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where VerifyBy>0 AND VerifyDate!='0000-00-00' AND VerifyDate!='1970-01-01' AND ApprBy=0 AND (ApprDate='0000-00-00' OR ApprDate='1970-01-01') AND CrBy=".$_REQUEST['ei']." AND ClaimMonth=".$resym['Month']." order by c.BillDate ASC"); $no3=1; while($rtupTA=mysql_fetch_assoc($stupTA)){ ?>	
					    <tr style="height:22px;">
						  <td><?php echo $no3; ?></td>
                        ...
                        <?php $no3++; } ?>
						<?php $stupTAT=mysql_query("select sum(FilledTAmt) as sFilledTAmt,sum(VerifyTAmt) as sVerifyTAmt from `y".$_SESSION['FYearId']."_expenseclaims` c inner join claimtype ct on c.ClaimId=ct.ClaimId where VerifyBy>0 AND VerifyDate!='0000-00-00' AND VerifyDate!='1970-01-01' AND ApprBy=0 AND (ApprDate='0000-00-00' OR ApprDate='1970-01-01') AND CrBy=".$_REQUEST['ei']." AND ClaimMonth=".$resym['Month']." order by c.BillDate ASC"); $rtupTAT=mysql_fetch_assoc($stupTAT); ?>
						 <tr style="height:22px;">
					      <td colspan="3" style="text-align:right;"><b>Total</b>&nbsp;</td>	
